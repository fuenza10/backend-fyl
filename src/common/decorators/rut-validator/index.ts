...
class IsValidRutConstraint implements ValidatorConstraintInterface {
	validate(value: string, args: ValidationArguments) {
		return validateRut(value); // Usa tu función validateRut aquí
	}
...
/**
 * Decorator that validates if a property value is a valid Chilean RUT (Rol Único Tributario).
 * 
...
export function IsValidRut(
  ...
  return (object: Record<string, unknown>, propertyName: string | symbol) => {
      registerDecorator({
